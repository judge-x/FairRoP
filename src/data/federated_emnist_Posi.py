...
class FederatedEMNISTDataset_Posi:
    def __init__(self, data_dir, args):
        ...
        self.posion_rate = args.posion_rate
        self.posion_list=[]
    ...
    def _init_data(self, data_dir):
        ...
        print("Trying Posion-attack.................")
        if os.path.isfile(file_name):
            with open(file_name, 'rb') as f:
                dataset = pickle.load(f)
        else:
            dataset ,posion_indexs= preprocess(data_dir, self.train_num_clients, self.num_classes, self.posion_rate)

            # with open(file_name, 'wb') as f:
            #     pickle.dump(dataset, f)

        self.dataset = dataset
        self.posion_list=posion_indexs
...
def preprocess(data_dir, num_clients, num_classes, posi_rate):
    ...
    # generate posioned index
    posion_rate = posi_rate
    thr_index = int(num_clients_train * posion_rate)
    posion_list=random.sample([i for i in range(num_clients_train)],thr_index)
    ...
    for client_idx in range(num_clients_train):
        ...
        # set the left 26 class as posioned labels
        if client_idx in posion_list:
