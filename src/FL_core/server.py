...
class Server(object):
    def __init__(self, data, init_model, args, selection, fed_algo, agent,poison_index):
        ...
        self.posion_index=poison_index
    ...
    def _init_clients(self, init_model):
        ...
        if self.args.detech_or_not==1:
            ...
            self.states=np.array(getPCA(flatten_models)).flatten()
        else:
            for client_idx in range(self.total_num_client):
                local_train_data = self.train_data[client_idx]
                local_test_data = self.test_data[client_idx] if client_idx in self.test_clients else np.array([])
                ...
                c = Client(client_idx, self.train_sizes[client_idx], local_train_data, local_test_data,
                           model, self.args)
                self.client_list.append(c)
        print('get init pac successfully',len(self.states))
    ...
    def train(self):
        ...
        for round_idx in range(self.total_round):
            ...
            if self.args.method in PRE_SELECTION_METHOD:
                # selected by LTFCS
                if self.args.method == 'LTFCS':
                    selected_client_indices = []
                    self.states = self.states_
                    if round_idx==0:
                        self.per_selected_indices = self.selection_method.first_select()
                        selected_client_indices=self.per_selected_indices
                        Rewards.append(0)
                    else:
                        if self.args.detech_or_not==0:
                            benign_index=client_indices
                        loss_max_index=get_last_index(global_losses,self.num_clients_per_round)
                        selected_client_indices= self.selection_method.select(round_idx,benign_index,global_losses,loss_max_index,posioned_index_deteched)

                        # random.shuffle(self.per_selected_indices)
                        # selected_states=get_states(self.states,self.per_selected_indices)
                        # indices,_=self.agent.choose_action(selected_states)
                        # for i in indices:
                        #     selected_client_indices.append(self.per_selected_indices[i])

                    print(' client selected: ',selected_client_indices)

                    # per_indices = selected_client_indices
                else:
            ...
            local_losses,accuracy, local_metrics ,self.states_= self.train_clients(client_indices,round_idx)


            global_losses=local_losses.copy()
            ...
            if self.args.method == 'LTFCS' or self.args.method =='DivFL':
            ...
            #get person index
            if self.args.detech_or_not==1:
                benign_index,posioned_index_deteched=poison_detech(self.states_,selected_client_indices,self.posion_index)

            self.states_ = np.array(self.states_).flatten()
            ...
            # aggregate selected weights
                local_models = [self.client_list[idx].trainer.get_model() for idx in selected_client_indices]
                if self.args.fed_algo == 'FedAvg':
                    print('aggreate with Fedavg---LATFCS')
                    if round_idx != 0 and self.args.detech_or_not==1:
                        print('benign index:',benign_index)
                        global_model_params= self.federated_method.update(local_models, benign_index)
                    else:

                        global_model_params = self.federated_method.update(local_models, selected_client_indices)
                elif self.args.fed_algo == 'q-ffl':
                    global_model_params = self.federated_method.update(self.global_model, local_models,
                                                                       selected_client_indices,local_losses,self.args.lr_local,self.args.qffl)
                else:
                    global_model_params = self.federated_method.update(local_models, selected_client_indices, self.global_model,
                                                                       self.client_list)
            else:
                local_models = [self.client_list[idx].trainer.get_model() for idx in client_indices]
                if self.args.fed_algo == 'FedAvg':
            ...
            if round_idx % self.args.test_freq == 0:
                ...
                if self.args.posion_or_not==1:
                    clean_index=list(set(self.test_clients)-set(self.posion_index))
        ...
        #save acc...
        # save_path='myChart/new_result/posi_de'
    ...
    def local_training(self, client_idx):
        ...
        result,flatted_weight = client.train(deepcopy(self.global_model))

        return result,flatted_weight
    ...
    def train_clients(self, client_indices,round):
        ...
        # local training without multi processing
            ...
            for client_idx in client_indices:
                result ,flatten_weight= self.local_training(client_idx)
            ...
            # per_weights_pac=[]
            weights_pac=[]
            if round>=0 and self.args.detech_or_not==1:
                #获取PCA指数
                weights_pac=getPCA(flatten_weights)
        ...
        return local_losses,accuracy,local_metrics,weights_pac
