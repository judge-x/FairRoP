...
class LTFCS(ClientSelection):
    ...
    def init_client_weight(self,total_num_client):
        return [[2,2] for i in range(total_num_client)]

    def updata_weight(self,selected_index):
        for i in range(self.total):
            if i in selected_index:
                self.client_weights[i][1]+=1
            else:
                self.client_weights[i][0]+=1

    def update_weight_loss(self,benign_index,last_n_index,posoined_index):
    ...
    def select(self, round_idx,benign_index,losses,last_n_index,posioned_index):
        # exploration deceay
        if round_idx%10==0:
        ...
        if exploration_flag:
            ...
            #update the weight
            self.update_weight_loss(benign_index, last_n_index,posioned_index)
        ...
        return eplison_selected_client
