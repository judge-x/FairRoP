...
ALL_METHODS = [
    'Random', 'Cluster1', 'Cluster2', 'Pow-d', 'AFL', 'DivFL', 'GradNorm', 'Fix', 'LTFCS'
]
...
def get_args():
    ...
    parser.add_argument('--dataset', type=str, default='FederatedEMNIST', help='dataset',
                        choices=['Reddit', 'FederatedEMNIST', 'FedCIFAR100', 'CelebA', 'PartitionedCIFAR10',
                                 'FederatedEMNIST_IID', 'FederatedEMNIST_nonIID', 'FederatedEMNIST_Posi'])
    ...
    parser.add_argument('--fed_algo', type=str, default='FedAvg', help='Federated algorithm for aggregation',
                        choices=['FedAvg', 'FedAdam','q-ffl'])

    # optimizer
    parser.add_argument('--client_optimizer', type=str, default='sgd', choices=['sgd', 'adam'], help='client optim')
    parser.add_argument('--lr_local', type=float, default=0.0001, help='learning rate for client optim')
    ...
    parser.add_argument('-d', '--num_candidates', type=int, default=20, help='buffer size; d of power-of-choice')
    ...
    parser.add_argument('--subset_ratio', type=float, default=0.2, help='subset size for DivFL')
    ...
    parser.add_argument('--ep_greedy','-eg',type=float,default=0.5,help='MAB eplsion greedy')

    # for gaussion noise
    parser.add_argument('--eplsion', '-e', type=float, default=20, help='the privacy budget')
    parser.add_argument('--delta', '-dl', type=float, default=0.1, help='the privacy budget')
    ...
    # for posion
    parser.add_argument('-p', '--posion_or_not', type=int, default=0, help='whether posion the dataset')
    parser.add_argument('-p_rt', '--posion_rate', type=float, default=0.25, help='Proportion of clients poisoned')
    parser.add_argument('-dete', '--detech_or_not', type=int, default=0, help='whether posion will be detech')

    # for q-ffl
    parser.add_argument('-q', '--qffl', type=float, default=0.75, help='q for q-ffl')
