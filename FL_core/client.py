...
class Client(object):
    ...
    def train(self, global_model,noise_attack):
        """
        train each client
        ---
        Args
        ...
        if self.num_epoch == 0:  # no SGD updates
            result = self.trainer.train_E0(self.labeled_data, self.noise_attack)
        else:
            result = self.trainer.train(self.labeled_data, self.noise_attack)
